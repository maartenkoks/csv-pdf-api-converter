# app.py (Definitieve versie met dynamische API route en verbeterde param parsing)
import os
import subprocess
import time
import pandas as pd
...
# Zorg dat je de correcte robust_csv.py gebruikt (van 15:29)
...
MULTI_SEARCH_PARAM_NAME = None; MULTI_SEARCH_COLS = []; COL_TYPES_CACHE = {}
COL_SAMPLES_CACHE = {}; NGROK_PROCESS = None; NGROK_URL = None

# --- OpenAI Client Initialisatie ---
openai_api_key = os.environ.get("OPENAI_API_KEY")
if not openai_api_key or not openai_api_key.startswith("sk-"):
    logger.warning("OPENAI_API_KEY not found/invalid. LLM features disabled.")
    client = None
else:
    try: client = OpenAI(api_key=openai_api_key); logger.info("OpenAI client initialized.")
    except Exception as e: logger.error(f"Failed to init OpenAI client: {e}", exc_info=True); client = None

# --- LLM Functie (met sample truncation) ---
def generate_column_descriptions_with_llm(context, columns, samples, max_sample_length=150):
    if not client: logger.error("OpenAI client unavailable."); return {}
    prompt_lines = [
    ...
    for col in columns:
        ...
        truncated_samples = [s[:max_sample_length] + "..." if len(s) > max_sample_length else s for s in sample_vals]
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[ {"role": "system", "content": "Generate JSON column descriptions."}, {"role": "user", "content": full_prompt} ],
        ...
        try:
            ...
            else: json_str = raw_content.strip().strip('```json').strip('`').strip()
            descriptions_dict = json.loads(json_str)
            if not isinstance(descriptions_dict, dict): raise ValueError("LLM response not dict.")
            ...
            if missing_cols: logger.warning(f"LLM missing descriptions for: {missing_cols}")
            logger.info(f"Parsed LLM descriptions for {len(final_descriptions)} columns.")
            return final_descriptions
        except Exception as json_err: logger.error(f"Failed parse/validate JSON: {json_err}\nContent: {raw_content}"); return {}
    except Exception as e: logger.error(f"OpenAI call error: {e}", exc_info=True); return {}
...
def upload_file():
    ...
    if not uploaded_file or uploaded_file.filename == '':
        logger.warning("Upload: No file selected."); return redirect(url_for('index'))
    ...
    if file_type == "csv":
        ...
        try:
            ...
            if df is None: logger.error(f"CSV cleaner failed: {temp_path}."); return redirect(url_for('index'))
            if df.empty and os.path.getsize(temp_path) > 0: logger.warning(f"CSV cleaner empty DF: {temp_path}.")
            ...
            COL_TYPES_CACHE = {}; COL_SAMPLES_CACHE = {}; SELECTED_COLS = []
            MULTI_SEARCH_PARAM_NAME = None; MULTI_SEARCH_COLS = []; COL_DESCRIPTIONS = {}
            ...
            if not DATAFRAME_STORAGE.empty:
                 COL_DESCRIPTIONS = generate_column_descriptions_with_llm(
                     context=dataset_context, columns=list(DATAFRAME_STORAGE.columns), samples=COL_SAMPLES_CACHE
                 )
                 logger.info(f"LLM descriptions: {len(COL_DESCRIPTIONS)} generated.")
            else: logger.info("Skipping LLM for empty DF.")

            return redirect(url_for("analyze"))
        except Exception as e: logger.error(f"Error processing CSV '{filename}': {e}", exc_info=True); return redirect(url_for('index'))
        finally:
            if 'temp_path' in locals() and os.path.exists(temp_path):
                try: os.remove(temp_path); logger.info(f"Removed temp CSV: {temp_path}")
                except OSError as e_rem: logger.error(f"Error removing temp CSV {temp_path}: {e_rem}")
        ...
        try:
            DATAFRAME_STORAGE = None; SELECTED_COLS = []; COL_DESCRIPTIONS = {}
            MULTI_SEARCH_PARAM_NAME = None; MULTI_SEARCH_COLS = []; COL_TYPES_CACHE = {}
            COL_SAMPLES_CACHE = {}; session.pop('current_dataset_slug', None)
...
def analyze_columns_for_display(df):
    ...
    if df is None or df.empty: COL_TYPES_CACHE = {}; COL_SAMPLES_CACHE = {}; return
    COL_TYPES_CACHE = {}; COL_SAMPLES_CACHE = {}
    for col in df.columns:
        try: unique_non_null = df[col].dropna().unique(); COL_SAMPLES_CACHE[col] = [str(v) for v in unique_non_null[:5]]
        except Exception as e: logger.warning(f"Unique vals err '{col}': {e}"); COL_SAMPLES_CACHE[col] = []; unique_non_null = pd.Series([])
        sample_serie = df[col].dropna().head(1000)
        if sample_serie.empty: COL_TYPES_CACHE[col] = "empty"; continue
        possible_bool_map = {'true': True, '1': True, 'yes': True, 'ja': True, 't': True,'false': False, '0': False, 'no': False, 'nee': False, 'f': False}
        unique_strs_lower = {str(x).strip().lower() for x in unique_non_null if pd.notna(x)}
        if unique_strs_lower and unique_strs_lower.issubset(possible_bool_map.keys()):
             if len({possible_bool_map[s] for s in unique_strs_lower}) <= 2: COL_TYPES_CACHE[col] = "boolean"; continue
        try:
            numeric_series = pd.to_numeric(sample_serie, errors='coerce')
            if numeric_series.notna().all():
                 is_close = ((numeric_series - numeric_series.round()).abs() < 1e-9).all()
                 if is_close: COL_TYPES_CACHE[col] = "integer" if sample_serie.astype(str).str.match(r'^-?\d+$').all() else "number"; continue
                 else: COL_TYPES_CACHE[col] = "number"; continue
            else: COL_TYPES_CACHE[col] = "string"; continue
        except: COL_TYPES_CACHE[col] = "string"; continue
        if col not in COL_TYPES_CACHE: COL_TYPES_CACHE[col] = "string"
...
def select_columns_and_configure_multi():
    ...
    if enable_multi:
        param_name = request.form.get("multi_param_name", "query").strip(); param_name_sanitized = re.sub(r'\W+', '', param_name) or "query"
        if param_name_sanitized in SELECTED_COLS: param_name_sanitized = f"{param_name_sanitized}_multi"; logger.warning(f"Multi param rename: '{param_name_sanitized}'.")
        MULTI_SEARCH_PARAM_NAME = param_name_sanitized; MULTI_SEARCH_COLS = request.form.getlist("multi_search_target_cols"); all_df_columns = list(DATAFRAME_STORAGE.columns)
        valid_multi_cols = [col for col in MULTI_SEARCH_COLS if col in all_df_columns]
        if len(valid_multi_cols) != len(MULTI_SEARCH_COLS): logger.warning(f"Invalid multi cols ignored: {set(MULTI_SEARCH_COLS)-set(valid_multi_cols)}"); MULTI_SEARCH_COLS = valid_multi_cols
        if not MULTI_SEARCH_COLS: logger.warning("Multi enabled, no valid cols. Disabling."); MULTI_SEARCH_PARAM_NAME = None
        else: logger.info(f"Multi configured: param='{MULTI_SEARCH_PARAM_NAME}', cols={MULTI_SEARCH_COLS}")
    else: MULTI_SEARCH_PARAM_NAME = None; MULTI_SEARCH_COLS = []; logger.info("Multi disabled.")
    all_df_columns = list(DATAFRAME_STORAGE.columns); valid_selected_cols = [col for col in SELECTED_COLS if col in all_df_columns]
    if len(valid_selected_cols) != len(SELECTED_COLS): logger.warning(f"Invalid selected cols ignored: {set(SELECTED_COLS)-set(valid_selected_cols)}"); SELECTED_COLS = valid_selected_cols
...
def generate_api():
    ...
    if MULTI_SEARCH_PARAM_NAME and MULTI_SEARCH_COLS:
        for col in MULTI_SEARCH_COLS: multi_col_details[col] = { 'description': COL_DESCRIPTIONS.get(col) or guess_description(col), 'type': COL_TYPES_CACHE.get(col, 'Unknown'), 'samples': COL_SAMPLES_CACHE.get(col, []) }
    return render_template( "generate.html", selected_cols=SELECTED_COLS, descriptions=final_descriptions, col_types=COL_TYPES_CACHE, samples=COL_SAMPLES_CACHE, ngrok_url=NGROK_URL, multi_param_name=MULTI_SEARCH_PARAM_NAME, multi_search_cols=MULTI_SEARCH_COLS, multi_col_details=multi_col_details )

# --- Functie: Guess Description ---
def guess_description(col_name):
    # Functie blijft ongewijzigd - zie vorig antwoord
    try:
        if not isinstance(col_name, str): col_name = str(col_name)
        original_col_name = col_name; c_lower = col_name.lower().strip()
        if not c_lower: return "Filter by this column (empty name)."
        id_pattern = r'(^id_|_id$|identifier|\Bid\B|_nr$|^nr_)'
        if re.search(id_pattern, c_lower): base_name = re.sub(id_pattern, '', original_col_name, flags=re.IGNORECASE).strip().strip('_- .'); return f"Filter by unique ID for '{base_name}'." if base_name else f"Filter by unique ID ('{original_col_name}')."
        if "city" in c_lower or "stad" in c_lower or "plaats" in c_lower or "gemeente" in c_lower: return f"Filter by city/municipality ('{original_col_name}')."
        if "zip" in c_lower or "postcode" in c_lower or "postal" in c_lower: return f"Filter by postal code ('{original_col_name}')."
        if "country" in c_lower or "land" in c_lower: return f"Filter by country ('{original_col_name}')."
        if "region" in c_lower or "provincie" in c_lower or "state" in c_lower or "gewest" in c_lower: return f"Filter by region/province/state ('{original_col_name}')."
        if "mail" in c_lower: return f"Filter by email ('{original_col_name}')."
        if "phone" in c_lower or "telefoon" in c_lower or "gsm" in c_lower: return f"Filter by phone number ('{original_col_name}')."
        if "url" in c_lower or "website" in c_lower or "link" in c_lower: return f"Filter by URL/website ('{original_col_name}')."
        if "date" in c_lower or "datum" in c_lower or "_dt" in c_lower or c_lower.endswith("day") or "time" in c_lower or "stamp" in c_lower or "jaar" in c_lower or "year" in c_lower or "maand" in c_lower or "month" in c_lower: return f"Filter by date/time ('{original_col_name}')."
        if "wage" in c_lower or "loon" in c_lower or "salaris" in c_lower or "salary" in c_lower or "amount" in c_lower or "price" in c_lower or "value" in c_lower or "waarde" in c_lower or "bedrag" in c_lower or "cost" in c_lower: return f"Filter by amount/price/wage ('{original_col_name}')."
        if "nummer" in c_lower or "number" in c_lower or "count" in c_lower or "aantal" in c_lower: return f"Filter by number/count ('{original_col_name}')."
        if "min" in c_lower: return f"Filter by minimum value ('{original_col_name}')." # Simplified min/max
        if "max" in c_lower: return f"Filter by maximum value ('{original_col_name}')."
        if "status" in c_lower or ("state" in c_lower and "status" in c_lower): return f"Filter by status/state ('{original_col_name}')." # Avoid conflict with state as region
        if "type" in c_lower or "soort" in c_lower: return f"Filter by type ('{original_col_name}')."
        if "lang" in c_lower or "taal" in c_lower: return f"Filter by language ('{original_col_name}')."
        if "code" in c_lower: return f"Filter by code ('{original_col_name}')."
        if "category" in c_lower or "categorie" in c_lower: return f"Filter by category ('{original_col_name}')."
        if "title" in c_lower or "titel" in c_lower: return f"Filter by title ('{original_col_name}')."
        if "desc" in c_lower or "omschr" in c_lower or "text" in c_lower or "tekst" in c_lower: return f"Filter by description/text ('{original_col_name}')."
        if "name" in c_lower or "naam" in c_lower: return f"Filter by name ('{original_col_name}')."
        if c_lower == 'y' or c_lower == 'n' or "flag" in c_lower or c_lower.startswith("is_") or c_lower.startswith("has_") or "publish" in c_lower: return f"Filter by boolean flag ('{original_col_name}')."
        return f"Filter by value in '{original_col_name}'." # Default
    except Exception as e: logger.error(f"Guess desc error '{col_name}': {e}"); return f"Filter by value in '{col_name}'."
...
def start_ngrok():
    ...
    ngrok_error = None # Initialize error for GET requests too
    if request.method == "POST":
        ...
        # --- Try block 1: Set Token ---
        try:
            logger.info("Configuring Ngrok authtoken...")
            result = subprocess.run([ngrok_cmd, "config", "add-authtoken", auth_token], check=True, capture_output=True, text=True, timeout=15)
        ...
        if NGROK_PROCESS and NGROK_PROCESS.poll() is None:
            try:
                ...
                NGROK_PROCESS = None
                NGROK_URL = None # Reset URL when stopping

        NGROK_URL = None # Ensure URL is reset before starting new tunnel
        time.sleep(0.5) # Shorter pause might be sufficient

        # --- Try Block 3: Start Ngrok and get URL ---
        try: # <------ BUITENSTE TRY STARTS
            logger.info("Starting Ngrok HTTP tunnel on port 5000...")
            startupinfo = None
            if os.name == 'nt': # Line 363 area
            ...
            time.sleep(4) # Give Ngrok time to start

            # Check if Ngrok stopped unexpectedly
            if NGROK_PROCESS.poll() is not None:
                ...
                NGROK_PROCESS = None # Ensure process is None
                # Belangrijk: Roep hier return aan om niet door te gaan naar de code na de try-except
                return redirect(url_for("start_ngrok"))

            # --- Try block 4 (nested): Get Tunnel URL via API ---
            # Dit blok blijft BINNEN het buitenste try blok
            try: # <------ BINNENSTE TRY STARTS
                ...
                https_tunnel = next((t for t in tunnels if t.get("proto") == "https" and "localhost:5000" in t.get("config", {}).get("addr", "")), None)
                ...
                if https_tunnel and https_tunnel.get("public_url"):
                    ...
                    # Redirect based on the file type that triggered Ngrok start
                    if file_type == "pdf":
                        return redirect(url_for("generate_pdf"))
                    elif file_type == "csv":
                        return redirect(url_for("generate_api"))
                    else:
                        logger.warning(f"Ngrok started, unknown file_type '{file_type}'. Redirecting index.")
                        return redirect(url_for("index"))
                else:
                    ...
                    if NGROK_PROCESS and NGROK_PROCESS.poll() is None: NGROK_PROCESS.terminate()
                    NGROK_PROCESS = None; NGROK_URL = None
                    return redirect(url_for("start_ngrok"))
            except requests.exceptions.RequestException as req_err: # Vang API request errors op
                logger.error(f"Failed to connect to Ngrok API: {req_err}")
                session['ngrok_error'] = "Failed get tunnel URL from Ngrok API."
                if NGROK_PROCESS and NGROK_PROCESS.poll() is None: NGROK_PROCESS.terminate()
                NGROK_PROCESS = None; NGROK_URL = None
                return redirect(url_for("start_ngrok"))
            # --- End Try Block 4 (nested) ---

        # --- DE CORRECT GEPLAATSTE EXCEPT VOOR BUITENSTE TRY ---
        except Exception as launch_e:
            logger.error(f"Error launching or managing Ngrok process: {launch_e}", exc_info=True)
            session['ngrok_error'] = f"Ngrok start error: {launch_e}"
            if NGROK_PROCESS and NGROK_PROCESS.poll() is None:
                try: NGROK_PROCESS.terminate()
                except Exception: pass
            NGROK_PROCESS = None
            NGROK_URL = None # Reset URL on error
            return redirect(url_for("start_ngrok"))
        # --- End Try Block 3 ---

    # Als het een GET request is, of als de POST redirect heeft gefaald (hoewel dat niet zou moeten)
    ngrok_error = session.pop('ngrok_error', None) # Haal evt. error op voor weergave
...
def generate_pdf():
    # Functie blijft ongewijzigd - zie vorig antwoord
    filename = session.get("uploaded_filename"); file_type = session.get("file_type")
    if not filename or file_type != "pdf": return redirect(url_for("index"))
    pdf_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    if not os.path.exists(pdf_path): logger.error(f"PDF '{filename}' not found."); return redirect(url_for("index"))
    try: local_base = request.host_url.rstrip('/'); pdf_rel = url_for("uploaded_file", filename=filename); local_url = f"{local_base}{pdf_rel}"; public_url = f"{NGROK_URL}{pdf_rel}" if NGROK_URL else None; logger.info(f"PDF URLs: Local={local_url}, Public={public_url}")
    except Exception as url_e: logger.error(f"PDF URL err: {url_e}"); local_url = "#err"; public_url = "#err" if NGROK_URL else None
    return render_template( "generate_pdf.html", local_pdf_url=local_url, public_pdf_url=public_url, filename=filename, ngrok_running=bool(NGROK_URL) )
...
def uploaded_file(filename):
    ...
    if safe_filename != filename: logger.warning(f"Unsafe path: {filename}"); return "Not Found", 404
    ...
    except FileNotFoundError: logger.error(f"File not found: {safe_filename}"); return "File not found", 404
